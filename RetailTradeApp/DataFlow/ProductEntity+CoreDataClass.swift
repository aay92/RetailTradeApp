...
//@objc(ProductEntity)
public class ProductEntity: NSManagedObject, Codable {
    
    required convenience public init(from decoder: Decoder) throws {
        /// first we need to extract managed object context to initialise
        guard let context = decoder.userInfo[.managedObjectContext] as? NSManagedObjectContext else { throw ContextError.NoContextFound }
